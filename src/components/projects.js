...
const Projects = (props) => {
  ...
  const ProjectDiv = ({ project, index }) => (
    <div key={index} className='col-md-4 s-project'>
      <div className='work-box'>
        ...
        <div className='work-content'>
          <div className='row'>
            ...
            <div className='col-sm-4'>
              <a href={isSupported ? project.imageWebP1 : project.image1} data-lightbox={project.dataLightbox}>
        ...
        {isSupported
          ? project.imagesWebP.map((img, ind) => <a key={ind} href={img} data-lightbox={project.dataLightbox} style={{ display: 'none' }}></a>)
          : project.images.map((img, ind) => <a key={ind} href={img} data-lightbox={project.dataLightbox} style={{ display: 'none' }}></a>)}
      </div>
    </div>
  );
  ...
  return (
    <div className='row'>
      {props.type === 'All'
        ? props.projects.map((project, index) => <ProjectDiv key={index} project={project} index={index} />)
        : props.projects.filter((p) => p.type === props.type).map((project, index) => <ProjectDiv key={index} project={project} index={index} />)}
    </div>
  );
};
